...
export class TypeOrmCrudService<T> extends CrudService<T> {
  ...
  public get findByIds(): Repository<T>['findByIds'] {
    return this.repo.findByIds.bind(this.repo);
  }

  public get save(): Repository<T>['save'] {
    return this.repo.save.bind(this.repo);
  }

  public get remove(): Repository<T>['remove'] {
    return this.repo.remove.bind(this.repo);
  }
  ...
  public async createOne(req?: CrudRequest, dto?: DeepPartial<T>): Promise<T> {
    if (req) {
      ...
      await this.checkPermissionBeforeSave(entity);
  ...
  public async updateOne(req: CrudRequest, dto: DeepPartial<T>): Promise<T> {
    ...
    await this.checkPermissionBeforeSave(toSave);
  ...
  public async replaceOne(req: CrudRequest, dto: DeepPartial<T>): Promise<T> {
    ...
    const replaced = await this.repo.save(
      plainToClass(this.entityType, toSave),
    );
  ...
  public async createBuilder(
    ...
    if ((options as any).crudOptions?.customJoin) {
      (options as any).crudOptions.customJoin(builder);
    }
  ...
  protected checkPermissionBeforeSave(dto: DeepPartial<T>) {
    if (!(dto as any).companyId) {
      return Promise.resolve();
    }
    const meta = this.repo.metadata;
    const relations = meta.relations.filter(
      (e) =>
        // e.isCascadeUpdate ||
        // e.isCascadeInsert ||
        e.isManyToOne || e.isOneToMany,
    );
    const tasks = [];
    for (const relation of relations) {
      const records = dto[relation.propertyPath];
      if (isArray(records)) {
        for (const record of records) {
          if (record.id) {
            // check permission
            const task = getRepository(relation.target)
              .findOne(record.id)
              .then((result: any) => {
                if (result && result.companyId === (dto as any).companyId) {
                  return Promise.resolve();
                } else {
      ...
      if (isObject(records)) {
        if (records.id) {
          const task = getRepository(relation.type)
            .findOne(records.id)
            .then((result: any) => {
              if (result && result.companyId === (dto as any).companyId) {
                return Promise.resolve();
  ...
  protected getRelationMetadata(
    ...
    try {
      ...
      if (this.entityRelationsHash.has(field)) {
        ...
        if (relationMetadata) {
          const { columns, primaryColumns } = this.getEntityColumns(
            relationMetadata,
          );
  ...
  protected setJoin(
    ...
    if (allowedRelation.path === 'sectionItems.section.file') {
      allowedRelation.path = 'section.file';
    }

    if (allowedRelation.path === 'sectionItems.section.exam') {
      allowedRelation.path = 'section.exam';
    }
